...
public class App2 {
    ...
    // Configuración de la base de datos (tal y como aparece en los ejemplos del
    // PDF)
    ...
    // 1 Listado básico: nombre y curso (SELECT -> executeQuery)
    ...
    // 2 Buscar por curso (SELECT con parámetro)
    ...
    // 4 Aprobados o suspensos (SELECT con condición)
    ...
    // 5 Mejor estudiante: se muestra el alumno con la nota más alta
    public static void mejorEstudiante() {
        // Consulta que devuelve la fila con la nota más alta (uso de LIMIT 1)
    ...
    // 6 Nueva matrícula (INSERT -> executeUpdate). Importante: setear parámetros
    // antes de ejecutar.
    public static void matricular() {
        ...
        try {
            conn = DriverManager.getConnection(URL, USER, PASSWORD);
            stmt = conn.prepareStatement(sql);
            // setear parámetros antes de ejecutar (según ejemplo del PDF)
